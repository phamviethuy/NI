...
'   channels using the digital change detection event.
'
' Instructions for running:
'   1.  Select the physical lines on the DAQ device to be read.
'   2.  Select the rising and falling edges on which to trigger a digital change
'       detection event.
'
' Steps:
'   1.  Create a new digital input task.
'   2.  Create a digital input channel.  Use one channel for all lines.
'   3.  Use the Timing.ConfigureChangeDetection method to set up change
'       detection timing.
'   4.  Add an event handler for the Digital Change Detection event.
'   5.  Start the task.
'   6.  Inside the callback, read the data and display it.
...
#End Region
    ...
    Private Sub startButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles startButton.Click
        ...
        Try
            ...
            AddHandler myTask.DigitalChangeDetection, AddressOf myTask_DigitalChangeDetection
    ...
    Private Sub myTask_DigitalChangeDetection(ByVal sender As Object, ByVal e As DigitalChangeDetectionEventArgs)
        Try
            Dim data As Boolean() = myDigitalReader.ReadSingleSampleMultiLine()
            DisplayData(data)
        Catch exception As DaqException
            myTask.Dispose()
            MessageBox.Show(exception.Message)
